            ...
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap', paddingBottom: '4px' }}>
                ...
                <button
                    ...
                    style={{
                        ...
                        borderRadius: '8px',
                        border: 'none',
                        background: activeSection === 'site_settings' ? '#4f46e5' : 'white',
                        color: activeSection === 'site_settings' ? 'white' : '#64748b',
                        fontWeight: '600',
                        cursor: 'pointer',
                        display: 'flex', alignItems: 'center', gap: '8px',
                        boxShadow: activeSection === 'site_settings' ? '0 4px 6px -1px rgba(79, 70, 229, 0.2)' : '0 1px 2px 0 rgba(0,0,0,0.05)',
                        whiteSpace: 'nowrap'
                    }}
                >
...
const SiteSettingsManager = () => {
    ...
    const fetchApiKey = async () => {
        try {
            const res = await api.get('/api/admin/config/gemini-api-key');
    ...
    const handleSave = async (e) => {
        ...
        setMessage('');
        try {
            await api.post('/api/admin/config/gemini-api-key', { key_value: apiKey });
    ...
    if (loading) return <div style={{ padding: '2rem', textAlign: 'center', color: '#64748b' }}>Loading Settings...</div>;

    return (
        <div style={{ padding: '1rem' }}>
            <div style={{ marginBottom: '2rem' }}>
                <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: '700', color: '#1e293b' }}>Site Configuration</h2>
                <p style={{ margin: '0.5rem 0 0 0', color: '#64748b' }}>Manage global application settings and API integrations.</p>
            </div>

            <div style={{ maxWidth: '600px', background: '#f8fafc', padding: '2rem', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                <form onSubmit={handleSave}>
                    <div style={{ marginBottom: '1.5rem' }}>
                        <label style={{ display: 'block', fontSize: '0.9rem', fontWeight: '700', color: '#475569', marginBottom: '0.5rem' }}>Gemini API Key</label>
                        ...
                        <p style={{ fontSize: '0.8rem', color: '#64748b', marginTop: '0.5rem' }}>
                            This key is used for site-wide AI features including transcription and the academic agent. 
                            It is stored securely in the database and overrides the environment variable.
                        </p>
                    </div>

                    {message && (
                        <div style={{ marginBottom: '1rem', padding: '10px', background: '#dcfce7', color: '#166534', borderRadius: '8px', fontSize: '0.9rem', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    >
                        {saving ? 'Saving...' : <><Zap size={18} /> Update Key</>}
                    </button>
                </form>
            </div>
            
            <div style={{ marginTop: '2rem', padding: '1.5rem', background: '#eff6ff', borderRadius: '12px', border: '1px solid #dbeafe', color: '#1e40af' }}>
                <div style={{ display: 'flex', gap: '12px' }}>
                    <AlertTriangle size={24} style={{ flexShrink: 0 }} />
                    <div>
                        <h4 style={{ margin: '0 0 0.5rem 0', fontWeight: '700' }}>Security Best Practice</h4>
                        <p style={{ margin: 0, fontSize: '0.9rem', lineHeight: '1.5' }}>
                            Never commit your API keys to GitHub. By using this admin screen, the key is saved directly to your private database. 
                            If you ever need to reset the system, ensure your <code>DATABASE_URL</code> is correctly configured in your Vercel/Production environment.
                        </p>
                    </div>
                </div>
            </div>
