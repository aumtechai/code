    ...
    const fetchHealth = async () => {
        ...
        try {
            ...
            if (res.data && res.data.modules) {
                ...
                setHealthError('Invalid response format from health service');
            }
        } catch (e) {
            console.error("Health check failed", e);
            setHealthError(e.response?.data?.detail || 'Failed to fetch EdNex health. Check connectivity.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
            ...
            {activeTab === 'status' && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    ...
                    {healthError && (
                        <div style={{ background: '#fef2f2', border: '1px solid #fee2e2', color: '#dc2626', padding: '1rem', borderRadius: '12px', marginBottom: '1.5rem', fontWeight: '500' }}>
                           <AlertCircle size={18} style={{ marginBottom: '-4px', marginRight: '8px' }} /> {healthError}
                        </div>
                    )}
