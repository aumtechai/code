...
-- 1. Create specialized schema for PrepAI
CREATE SCHEMA IF NOT EXISTS prepnex;

-- 2. Students Profile Table
CREATE TABLE IF NOT EXISTS prepnex.students (
...
CREATE TABLE IF NOT EXISTS prepnex.academic_roadmaps (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    student_id UUID REFERENCES prepnex.students(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS prepnex.profile_activities (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    student_id UUID REFERENCES prepnex.students(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS prepnex.test_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    student_id UUID REFERENCES prepnex.students(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS prepnex.ai_context (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    student_id UUID REFERENCES prepnex.students(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS prepnex.coaching_sessions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    student_id UUID REFERENCES prepnex.students(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_prepnex_student_id ON prepnex.students(user_id);
CREATE INDEX IF NOT EXISTS idx_prepnex_roadmaps_student ON prepnex.academic_roadmaps(student_id);
CREATE INDEX IF NOT EXISTS idx_prepnex_activities_student ON prepnex.profile_activities(student_id);
