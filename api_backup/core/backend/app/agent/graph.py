...
def router_node(state: AgentState):
    ...
    if any(word in last_msg for word in ["fail", "grade", "assignment", "study", "calculus", "chemistry", "gpa", "cumulative"]):
        return {"next_step": "tutor"}
    elif any(word in last_msg for word in ["drop", "register", "financial", "financ", "bill", "tuition", "status", "deadline", "transcript", "fee", "liability", "hold"]):
...
async def tutor_agent(state: AgentState):
    ...
    # AI Generation
    from app.config_utils import get_db_session, get_gemini_api_key
    with get_db_session() as db_session:
        api_key = get_gemini_api_key(db_session)
